...
From this, the estimated covariance matrix of the coefficients can be calculated by $\hat{\sigma}^2 ({\bf X}^T {\bf X})^{-1}$. The estimate of the covariance matrix for `model1` is
```{r}
model1$covmatrix
```
...
### Sifnificance of the regression 
...
In the $\chi^2$-test we are testing "significance of regression": $H_0: \beta_2=\beta_2=\cdots\beta_k=0 \text{ vs. }.H_1$: at least one different from zero.This means we test if at least one of the regression parameters (in addition to the intercept) is different from 0.
...
The parameter estimates differ because the model matrix $X$ changes as we add or remove terms. 
MÅ SVARES PÅ
When does the parameter estimates differ?

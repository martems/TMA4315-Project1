...
### $\hat{\sigma}^2$, covariance matrix and standard errors

The estimate for $\sigma^2$ is found by the restricted maximum likelihood estimator
$$
\hat{\sigma}^2 = \frac{\text{SSE}}{n - p},
$$
where $\hat{\sigma}$ is the residual standard error. 

From this, the estimated covariance matrix of the parameter estimates can be calculated by $\hat{\sigma}^2 ({\bf X}^T {\bf X})^{-1}$. The estimate of the covariance matrix for `model1` is
```{r}
model1$covmatrix
```

The standard errors of the intercept and regression coefficient are found by taking the square root of the covariance matrix, or ${\sqrt{c_{jj}}\hat{\sigma}}$, where $c_{jj}$ is diagonal element $j$ of $({\bf X}^T{\bf X})^{-1}$. For `model1` this gives the following
```{r}

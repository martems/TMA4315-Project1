...
model1$SST
```
The degrees of freedom is $n-p$. The degrees of freedom for this model with $p=2$ is:
```{r}
model1$df
```
...
From this, the estimated covariance matrix of the parameter estimates can be calulated by $\hat{\sigma}^2({\bf X}^T{\bf X})^{-1}$. 

We perform a $z$-test to test the significance of the estimated coefficients. The $z$-values for each coefficient $\hat{\beta_j}$ is caluclated by $\frac{\hat{\beta_j}}{\sqrt(c_{jj})\hat{\sigma}}$, where $c_{jj}$ is diagonal element $j$ of $({\bf X}^T{\bf X})^{-1})$. The associated p-values is found by the commands
```{r eval = FALSE}
pvalue_ztest <- 2*pnorm(abs(z),lower.tail = FALSE)
```

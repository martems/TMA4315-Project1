...
## Assumptions of multiple linear regression (MLR) analysis
 If we want to perform MLR analysis, the following must hold true:
   ...
   2. The error needs to have equal and uncorrelated variance for each covariate. $\text{Cov}\varepsilon = \sigma^2 I$.
   3. Errors need to be additive (see formula related to pt. 1).
   4. Errors need to be normally distributed. $\varepsilon ~ N_n(0, \sigma^2 I)$.
...
We fit a linear regression model to the data with wages as the response with education and age as
predictors.
...
Fit two simple linear regression with only age and only education to compare
